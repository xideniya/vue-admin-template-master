<template>
  <div>
    ...
    <el-table v-loading="listLoading" border stripe style="width: 960px" :data="roles" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="300" align="center">
        <template slot-scope="{ row }">
          ...
          <el-button v-if="row.edit" size="mini" type="primary" icon="el-icon-check" title="确定" @click="updateRole(row)"></el-button>
          <el-button v-if="!row.edit" size="mini" type="primary" icon="el-icon-edit" title="修改角色" @click="row.edit = true"></el-button>
    ...
    <!-- 分页组件 -->
    <el-pagination :current-page="page" :total="total" :page-size="limit" :page-sizes="[5, 10, 20, 30, 40, 50, 100]" style="padding: 20px 0" layout="prev, pager, next, jumper, ->, sizes, total" @current-change="getRoles" @size-change="handleSizeChange"></el-pagination>
  </div>
</template>
