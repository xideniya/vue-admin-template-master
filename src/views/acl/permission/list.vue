    ...
    <el-table border style="margin-bottom: 20px" :data="menuPermissionList" :expand-row-keys="expandKeys" row-key="id">
      ...
      <el-table-column label="操作">
        <template slot-scope="{ row }">
          <el-button :disabled="row.level === 4" type="primary" icon="el-icon-plus" size="mini" :title="getAddTitle(row.level)" @click="toAddPermission(row)"></el-button>
          <el-button type="primary" icon="el-icon-edit" size="mini" :disabled="row.level === 1" :title="row.level === 4 ? '修改功能' : '修改菜单'" @click="toUpdatePermission(row)"></el-button>
          <el-button :disabled="row.level === 1" type="danger" icon="el-icon-delete" size="mini" title="删除" @click="removePermission(row)"></el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    */
    addOrUpdatePermission() {
      this.$refs.permission.validate(async (valid) => {
        if (valid) {
          const { ...perm } = this.permission // pname不需要携带
